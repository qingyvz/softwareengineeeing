...
public class DocumentPdfMetaEntity {
    ...
    /** 原始 PDF 中最高的对象编号。增量更新中新增对象从 {@code lastObjectId + 1} 开始顺序分配。*/
    private int lastObjectId;

    /** 预先量算的水印增量更新附录大小（字节）。因 userId 为固定长度、时间戳格式固定，图像以 Raw 编码，附录大小仅取决于 PDF 结构（页数、每页字典大小），与具体用户无关
     * 0 表示尚未计算（流式预览功能未启用）
     */
    @Field("appendix_size")
     ...
     * 增量更新附录通过覆盖此对象 ID 来"填充"真实水印内容，无需修改任何 Page Dict 或 Content Stream
     */
